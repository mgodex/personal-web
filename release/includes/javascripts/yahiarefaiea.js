 ...
 *  mgod5288@gmail.com
 *  
 *  Last update on: 2026/6/2
 *  ©2026 mGod. all rights reserved.
 */
...
window.RealProgress = function () {
  ...
  function _isUntracked(subject) {
    ...
    for (var i = 0; i < list.length; i++) {
      if (typeof list[i] == "string" && list[i] == subject || typeof list[i] == "object" && list[i] instanceof RegExp && typeof subject == "string" && subject.match(list[i])) {
        return true;
      }
    }
  ...
  !function (t) {
    ...
    var t,
      ...
      var n,
          ...
          v = u.advanced.extraDraggableSelectors ? e(!A() || top.document).add(e(u.advanced.extraDraggableSelectors)) : e(!A() || top.document);
...
$(document).ready(function () {
  var list = $('.list');
  var link = list.find('li.listItem a, li.archived a'); //  LISTEN
...
var Identity = {
  ...
  //  ABORT
  abort: function () {
    if (Identity.status == 'robot') $(Identity.id).removeClass('robot');else if (Identity.status != 'loading' && Identity.processing != true) {
...
var Yahia = {
  load: function (source) {
    var imgData;
    if (source == 'home') imgData = 'includes/images/mgod.png';else if (source == 'bucket') imgData = 'includes/images/mgod.png';else if (source == 'projectsAll') imgData = 'includes/images/mgod.png';else if (source == 'thoughtsAll') imgData = 'includes/images/mgod.png';else if (source == 'notFound') imgData = 'includes/images/mgod.png';
...
var Router = {
  ...
  route: function (location, callback) {
    Identity.work();
    Router.location = Router.processLocation(location); //	ROUTES
  ...
  callback: function (callback) {
    ...
    setTimeout(function () {
      Identity.stop(function () {
        ...
        if (!md.mobile()) Yahia.load(Router.wrapper[0]); //  CALLBACKS
...
Router.routes = function (callback) {
  Router.wrapper = [];
  var location = Router.location.split('/').filter(Boolean); //  HOME

  if (location[0] === undefined) Router.push('home'); //  BUCKET
  else if (location[0] == 'bucket' && location[1] === undefined) Router.push('bucket'); //  BUCKET ALL
    else if (location[0] == 'bucket' && location[1] == 'all' && location[2] === undefined) Router.push('bucketAll'); //  PROJECTS
      else if (location[0] == 'projects' && location[1] === undefined) Router.push('projectsAll'); //  THOUGHTS
        else if (location[0] == 'thoughts' && location[1] === undefined) Router.push('thoughtsAll'); //  NOT FOUND
          else Router.push('notFound'); //  CALLBACK
